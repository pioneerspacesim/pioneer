...
define_model('hhbridge', {     
	...
	if lod > 1 then
		...
		elseif lod > 2 then
			texture('bridge2M.png')
		else
			texture('bridge2L.png')
		end
...
define_model('hh', {     
   	...
   	if lod > 1 then
		...
		elseif lod > 2 then
			texture('body_topM.png')
		else
			texture('body_topL.png')
		end
	...
	if lod > 2  then
		...
		if get_arg(10) > 0 then
			use_material('darksteel')
			texture(nil)
			call_model('hhfgun',v(0,0,36*get_arg(0)),v(1,0,0),v(0,1,0),1)
		end		
