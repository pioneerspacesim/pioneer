...
define_model('rattle', {
	...
	static = function(lod)
		...
		if lod > 3 then
            ...
            texture('tile0s.png',v(-.05,.265,0),v(0,0,.9), v(0,.5,0))
		end
		...
		if lod > 3 then
		    ...
		    texture('tile0s.png', v(.05,.21,0),v(.5,0,0),v(0,0,1.41))
		end
        ...
        if lod > 3 then
	        ...
	        texture('tile0s.png', v(.05,.935,0),v(.5,0,0),v(0,0,1.41))
	    end
		...
		if lod > 3 then
		    ...
		    texture('tile0s.png', v(-.65,.51,0), v(.41,-.21,0), v(0,0,1.04))
		end
		...
		if lod > 3 then
		    ...
		    texture('tile0s.png', v(0,.48,0),v(.692,0,0),v(0,0,.58))
		end
...
define_model('rattle_pad_br', {
	...
	static = function(lod)
		...
		if lod > 3 then
		    ...
		    texture('pad_brs.png',v(.49,.5,0), v(.4,-.02,0), v(0,0,-1.15))
		end
...
define_model('rattle_pad_f', {
	...
	static = function(lod)
		...
		if lod > 3 then
		    ...
		    texture('pad_fs.png',v(.49,.5,0), v(.4,-.02,0), v(0,0,-1.15))
		end
		cylinder(6, v(0,0,0), v(0,.3,0), v(.5,0,1), 1.05)
	end,
 	...
 	static = function(lod)
		...
		if lod > 1 then
			...
			if lod > 2 then
			    ...
			    texture('tex1s.png',v(0,.5,0), v(.05,0,0), v(0,.08,0))
			end
    		...
    		if lod > 3 then
				...
				texture('socket_s.png', v(0,0,0), v(.5,0,0), v(0,-.5,0))
			end
			quad(v40,v41,v43,v42)
    		if lod > 3 then
				texture('socket_l.png', v(0,0,0), v(0,0,1), v(0,-.5,0))
			else
				texture('socket_s.png', v(0,0,0), v(0,0,1), v(0,-.5,0))
			end
			...
			if lod > 3 then
			    ...
			    texture('v_glow_s.png', v(0,.6,0), v(0,0,1), v(0,-.9,0))
			end
