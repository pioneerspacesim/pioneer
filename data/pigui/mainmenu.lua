...
local mainButtonSize = vector2.new(400,46) * (ui.screenHeight / 1200)
local dialogButtonSize = vector2.new(150,46) * (ui.screenHeight / 1200)
...
local function confirmQuit()
	...
	ui.withStyleColors({["WindowBg"] = colors.commsWindowBackground}, function()
		...
		ui.window("MainMenuQuitConfirm", {"NoTitleBar", "NoResize", "AlwaysAutoResize"}, function()
			...
			ui.dummy(vector2.new(5,15)) -- small vertical spacing
			ui.dummy(vector2.new(30, 5))
			ui.sameLine()
...
local function showMainMenu()
	...
	local winPos = vector2.new(ui.screenWidth - mainButtonSize.x - 100, ui.screenHeight/2 - (buttons * mainButtonSize.y)/2 - (2*mainButtonSize.y)/2 - 8)

	ui.setNextWindowPos(vector2.new(110,65),'Always')
		...
		ui.withStyleColors({["WindowBg"]=colors.transparent}, function()
			ui.window("shipinfoWindow", {"NoTitleBar","NoResize","NoFocusOnAppearing","NoBringToFrontOnFocus","AlwaysAutoResize"}, function()
				...
				if mn then
					local sd = ShipDef[mn]
					if sd then
						ui.addFancyText(vector2.new(ui.screenWidth / 3, ui.screenHeight / 5.0 * 4), ui.anchor.center, ui.anchor.bottom, {{text=sd.name, color=colors.white, font=orbiteer.large}}, colors.transparent)
						ui.addFancyText(vector2.new(ui.screenWidth / 3, ui.screenHeight / 5.0 * 4.02), ui.anchor.center, ui.anchor.top, {{text=lui[sd.shipClass:upper()], color=colors.white, font=orbiteer.medium}}, colors.transparent)
					end
				end
			end)
		end)
	...
	ui.withFont("orbiteer", 16 * (ui.screenHeight/1200),
							function()
								ui.setNextWindowPos(vector2.new(ui.screenWidth - ui.calcTextSize(build_text).x * 1.2,ui.screenHeight - 50), 'Always')
								ui.withStyleColors({["WindowBg"] = colors.transparent}, function()
