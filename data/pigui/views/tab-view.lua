...
function PiGuiTabView.renderTabView(self)
	...
	if (tab.showView) then
		ui.withStyleColorsAndVars(styleColors, styleVars, function()
			...
			ui.window(self.name .. "-Content", mainWindowFlags, function()
	...
	if (tab.err) then
		...
		ui.withStyleColorsAndVars(styleColors, styleVars, function()
			ui.window(self.name .. "-TabError", {"NoResize", "NoTitleBar"}, function()
	...
	ui.withFont(orbiteer.xlarge, function()
		...
		ui.window(self.name .. "-Name", staticButtonFlags, function()
			ui.text(tab.name)
		end)
	end)
	...
	ui.withStyleVars({WindowPadding = bigButtonWindowPadding, ItemSpacing = bigButtonItemSpacing}, function()
		ui.window(self.name .. "-Buttons", staticButtonFlags, function()
			for i, v in ipairs(self.tabs) do
