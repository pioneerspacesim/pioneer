...
if not stationView then
		...
		ui.withFont(pionillium.medlarge.name, self.style.fontSize, function()
			ui.withStyleVars({WindowPadding = self.style.inventoryPadding, ItemSpacing = self.style.itemSpacing}, function()
				ui.child("shipInventoryContainer", Vector2(0, 0), nil, useWindowPadding, function()
					...
					local fmt = "{} {} / {} {}" % {
						ui.Format.Cargo(player.usedCargo, 1), l.USED,
						ui.Format.Cargo((player.cargo_cap - player.usedCargo), 1), l.FREE
					}
					ui.gauge(gaugePos, player.usedCargo, '', fmt, 0, player.cargo_cap, nil, false, colors.gaugeEquipmentMarket, '', gaugeWidth, ui.getTextLineHeight())
	...
	ui.registerHandler("StationView", function(delta_t, refresh)
		stationView:renderTabView(refresh)
