...
ui.registerHandler(
	'init',
	function(progress)
		ui.setNextWindowPos(vector2.new(ui.screenWidth / 2 - window_width / 2, ui.screenHeight/3*2), "Always")
		ui.setNextWindowSize(vector2.new(window_width,200), "Always")
		ui.withFont("orbiteer", 18, function()
									ui.withStyleColors( {["WindowBg"] = transparent }, function ()
											ui.window("test", {"NoTitleBar", "NoResize", "NoMove"}, function()
																	...
																	ui.dummy(vector2.new((window_width - agesize.x) / 2, 0))
																	ui.sameLine()
																	ui.text(agestring)

																	ui.dummy(vector2.new(15,15))

																	ui.withStyleColors({ ["PlotHistogram"] = highlight, ["FrameBg"] = base }, function()
																			ui.progressBar(progress, vector2.new(window_width - 20, ui.screenHeight / 43), "") -- 1080 / 43 -> 25
																	end)
											end)
									end)
		end)
		local logosize = (ui.screenHeight / 2.5)
		local leftup = vector2.new(ui.screenWidth/2 - logosize/2, ui.screenHeight/7)
		ui.setNextWindowPos(leftup, "Always")
		ui.setNextWindowSize(vector2.new(logosize, logosize), "Always")
		ui.withStyleColors( {["WindowBg"] = transparent }, function ()
				ui.window("logo", {"NoTitleBar", "NoResize", "NoMove"}, function ()
										local size = vector2.new(logosize,logosize)
										local pos = vector2.new(0,0) -- vector2.new(ui.screenWidth / 2, ui.screenHeight / 3 * 2)
										local offset = ui.getWindowPos()
										ui.addImage(logo, pos + offset, pos + size + offset, vector2.new(0.0, 0.0), vector2.new(1.0, 1.0), Color(255,255,255))
				end)
		end)
end)
