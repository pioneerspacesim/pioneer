...
local onUpdateBB = function (station)
	for ref,ad in pairs(ads) do
		if not flavours[ad.flavour].localscout
			and ad.due < Game.time + 5*24*60*60 then -- 5 days
			ad.station:RemoveAdvert(ref)
		elseif flavours[ad.flavour].localscout
			and ad.due < Game.time + 2*24*60*60 then -- 2 days
			ad.station:RemoveAdvert(ref)
		end
	end
	if Engine.rand:Integer(3*24*60*60) < 60*60 then    -- 72 h < 1 h   ~1 new ad in 3 days
		makeAdvert(station)
	end
end
