...
-- range of possible resolution values generated by the mission
local orbital_max_resolution = 125
local orbital_min_resolution = 35
-- range of possible coverage values generated by the mission
local orbital_max_coverage = 0.8
local orbital_min_coverage = 0.1

-- approximate "normal" body radius used to scale coverage to the body being scanned
local orbital_nominal_radius = 2500

-- reward scaling per kilometer-width of body coverage
local orbital_reward_per_km = 0.05
-- reward scaling by resolution
local orbital_reward_resolution_max = 0.5
local orbital_reward_resolution_min = 8.0
...
local function calcOrbitalScanMission(sBody, difficulty, reward)
	...
	local coverage = Engine.rand:Number(orbital_min_coverage, orbital_max_coverage)
	local resolution = Engine.rand:Number(orbital_min_resolution, orbital_max_resolution) / realDifficulty

	local radiusKm = sBody.radius / 1000
	local radiusScalar = orbital_nominal_radius / radiusKm
	...
	if radiusScalar > 1.0 then
		...
		resolution = resolution * (1.0 + math.log(radiusKm / orbital_nominal_radius, 10) * 0.5)
	end

	coverage = math.min(coverage * realDifficulty, 1.0)

	local rewardAmount = reward
		* orbital_reward_per_km * (math.pi * radiusKm * coverage)
		* math.lerp(orbital_reward_resolution_min, orbital_reward_resolution_max,
			math.invlerp(orbital_min_resolution, orbital_max_resolution, resolution))
...
local filterBodyOrbital = function(sBody)
...
local makeAdvert = function (station)
	local reward, coverage, resolution, due, nearbysystem
	local location						  -- mission body
	local missionBody ---@type SystemBody
	...
	if flavour.localscout then  -- local system
		...
		local info = calcOrbitalScanMission(missionBody, difficulty, flavour.reward)

		coverage = info.coverage
		resolution = info.minResolution

		due = Game.time + days * MissionUtils.Days + MissionCalc:CalcTravelTime(station, location, urgency, 0.1)
		reward = info.reward + MissionCalc:CalcReward(station, location, urgency, difficulty - 1.0, 0.2)
	else                                   -- remote system
