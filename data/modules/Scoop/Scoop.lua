...
local placeAdvert = function (station, ad)
		...
		if ad.reward < 0 then
			...
			local body = station.path and station.path:GetSystemBody()
			local parent = body and body.parent
			local p = parent and parent.path
			ref = station:AddAdvert({
				title       = l["ADTITLE_" .. ad.id],
				description = ad.desc,
				icon        = ad.id == "RESCUE" and "searchrescue" or "haul",
				due         = ad.due,
				dist        = (p and p.path == ad.planet) and parent:GetPhysicalRadius() * 3 or nil,
				reward      = ad.reward,
