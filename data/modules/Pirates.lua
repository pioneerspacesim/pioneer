...
local onFrameChanged = function (body)
	...
	local superType = body.frameBody.superType
	local path = body.frameBody.path
	local systembody = path:GetSystemBody ()
	local planetPopulation = systembody.population

	-- XXX number should be some combination of population, lawlessness,
	-- proximity to shipping lanes, etc
	
	local modifiers = (lawlessness + (population / 100) - (planetPopulation / 100))
	...
	while max_pirates > 0 and Engine.rand:Number(1) < lawlessness do
		...
		local ship = Space.SpawnShipNear(shipid, body, 10, 30)
		ship:AddEquip(default_drive)
		ship:AddEquip(laser)
		ship:AIKill(Game.player)
	end
