...
define_model('conny_piston_f', {
	...
	dynamic = function(lod)
		...
		if lod > 1 then
			texture('lmrmodels/ships/constrictor/metal.png')
			use_material('chrome')
		end
...
define_model('conny_w_front', {
	...
	dynamic = function(lod)
		if get_animation_position('WHEEL_STATE') ~= 0 then
			...
			if lod > 1 then
				use_material('inside')
				texture('lmrmodels/ships/constrictor/iron.png',v(.5,.5,0),v(.1,0,0),v(0,0,1))
	...
	dynamic = function(lod)
		if get_animation_position('WHEEL_STATE') ~= 0 then
			...
			if lod > 1 then
				...
				use_material('chrome')
				texture('lmrmodels/ships/constrictor/metal.png',v(.5,.5,0),v(0,0,.5),v(0,1,0))
		...
		if lod > 2 then
			...
			if get_equipment('LASER', 1) then
				...
				if get_equipment('LASER', 1) == 'PULSECANNON_DUAL_1MW' then
					texture('lmrmodels/ships/constrictor/iron.png')
...
define_model('conny_extra_0', {
	...
	dynamic = function(lod)
		...
		use_material('glow')
		texture('lmrmodels/ships/constrictor/flow_1.png',v(0,trans,0),v(0,0,.05),v(1,0,0))
		...
		--texture('lmrmodels/ships/constrictor/flow_2.png',v(0,-trans,0),v(0,.1,0),v(1,0,0))
...
define_model('conny', {
	...
	dynamic = function(lod)
		...
		if lod > 2 then
			local reg = get_label()
			texture('lmrmodels/ships/constrictor/washed.png',v(.5,.5,0),v(.5,0,0),v(0,0,.5))
			use_material('text')
