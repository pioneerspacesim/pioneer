...
define_model('ips_dash', {
	...
	dynamic = function(lod)
		...
		if timer < .17 then
			texture('lmrmodels/ships/ip_shuttle/dash_lit_01.png', v(0.5,0.4,0), v(-.14,0,0), v(0,1.8,0))
		else
			if timer < .34 then
				texture('lmrmodels/ships/ip_shuttle/dash_lit_02.png', v(0.5,0.4,0), v(-.14,0,0), v(0,1.8,0))
			else
				if timer < .51 then
					texture('lmrmodels/ships/ip_shuttle/dash_lit_03.png', v(0.5,0.4,0), v(-.14,0,0), v(0,1.8,0))
				else
					if timer < .68 then
						texture('lmrmodels/ships/ip_shuttle/dash_lit_04.png', v(0.5,0.4,0), v(-.14,0,0), v(0,1.8,0))
					else
						if timer < .85 then
							texture('lmrmodels/ships/ip_shuttle/dash_lit_05.png', v(0.5,0.4,0), v(-.14,0,0), v(0,1.8,0))
						else
							if timer > .84 then
								texture('lmrmodels/ships/ip_shuttle/dash_lit_06.png', v(0.5,0.4,0), v(-.14,0,0), v(0,1.8,0))
							end
						end
					end
				end
...
define_model('ip_shuttle', {
	...
	dynamic = function(lod)
		...
		if lod > 1 then
			if lod > 2 then
				texture('lmrmodels/ships/ip_shuttle/ips_bottom.png', v(0.5,0.5,0), v(0.162,0,0), v(0,0,0.455))
			end
			if get_animation_position('WHEEL_STATE') ~= 0 then
				...
				texture('lmrmodels/ships/ip_shuttle/shut02.png', v(0.5,0.5,0), v(0,0.4,0), v(0,0,0.8))
				use_material('anth')
				xref_quad(v40,v41,v42,v43) -- uc cage top
				texture('lmrmodels/ships/ip_shuttle/shut02.png', v(0,0,0), v(0,0.8,0), v(0,0,0.2) )
				xref_quad(v32,v40,v43,v35) -- uc cage sides
				xref_quad(v33,v34,v42,v41)
				texture('lmrmodels/ships/ip_shuttle/shut02.png', v(0,0,0), v(0.4,0,0), v(0,0.8,0) )
