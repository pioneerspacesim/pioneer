...
define_model('rattle_uc', {
	...
	dynamic = function(lod)
		...
		if get_animation_position('WHEEL_STATE') ~= 0 then
			...
			use_material('chrome')
			texture('lmrmodels/ships/sidewinder/alu_tiled.png', v(0,.4+2*trans,0), v(.5,0,0),v(0,-.5,0))
...
define_model('rattle_dash', {
	...
	dynamic = function(lod)
		...
		if timer < .17 then
			texture('lmrmodels/ships/sidewinder/dash_lit_01.png', v(.5,.02,0), v(.11,0,0), v(0,0,-4.5))
		else
			if timer < .34 then
				texture('lmrmodels/ships/sidewinder/dash_lit_02.png', v(.5,.02,0), v(.11,0,0), v(0,0,-4.5))
			else
				if timer < .51 then
					texture('lmrmodels/ships/sidewinder/dash_lit_03.png', v(.5,.02,0), v(.11,0,0), v(0,0,-4.5))
				else
					if timer < .68 then
						texture('lmrmodels/ships/sidewinder/dash_lit_04.png', v(.5,.02,0), v(.11,0,0), v(0,0,-4.5))
					else
						if timer < .85 then
							texture('lmrmodels/ships/sidewinder/dash_lit_05.png', v(.5,.02,0), v(.11,0,0), v(0,0,-4.5))
						else
							if timer > .84 then
								texture('lmrmodels/ships/sidewinder/dash_lit_06.png', v(.5,.02,0), v(.11,0,0), v(0,0,-4.5))
							end
						end
					end
				end
