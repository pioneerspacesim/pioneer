define_orbital_station {
	...
	ship_dock_anim = function(port, stage, t, from, ship_aabb)
		local baypos = { v(-83,900,0), v(-100,900,100),
			v(100,900,100), v(150,900,0) }
		if stage == 2 then
			return { vlerp(t, from, v(0,900,0)), v(1,0,0), v(0,0,1) }
		elseif stage == 3 then
			return { v(0,900,0), v(1,0,0), v(0,1,0) }
		elseif stage == 4 then
			return { vlerp(t, v(0,900,0), baypos[port]), v(1,0,0), v(0,1,0) }
		elseif stage == -1 then
			return { vlerp(t, baypos[port], v(0,900,0)), v(1,0,0), v(0,1,0) }
		elseif stage == -2 then
			return { v(0,900,0), v(-1,0,0), v(0,0,-1) }
		elseif stage == -3 then
			return { vlerp(t, v(0,900,0), v(0,600,0)), v(-1,0,0), v(0,0,-1) }
		end
	end,
