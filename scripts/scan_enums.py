...
def write_license_header(fl):
    fl.write('/* Copyright © 2008-2026 Pioneer Developers. See AUTHORS.txt for details */\n')
...
def main():
    ...
    oparse.add_option('--meta', type="string", dest="metafile",
          help="Specify the meta .lua file to write enum information to. " +
               "Defaults to <outfile>/../data/meta/Constants.lua")
    ...
    if options.outfile == '-':
        ...
        if options.metafile is None:
            dirname = os.path.dirname(options.outfile)
            mpath = os.path.normpath(dirname + '/../data/meta/Constants.lua')
        else:
            mpath = options.metafile

        with open(hpath, 'w') as fl:
            write_header(enums, fl)
        with open(cpath, 'w') as fl:
            write_tables(enums, headers, os.path.basename(hpath), fl)
        with open(mpath, 'w') as fl:
            write_meta(enums, fl)
