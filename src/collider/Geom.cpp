...
void Geom::Collide(Geom *b, void (*callback)(CollisionContact *)) const
{
	...
	{
		PROFILE_SCOPED_DESC("Old Collision")
	...
	{
		PROFILE_SCOPED_DESC("New Collision")
		...
		/* Collide this geom's edges against tri-mesh of geom b */
		transTo = b->m_invOrient * m_orient;
		this->CollideEdgesWithTrisOf2(contacts, max_contacts, b, transTo);

		/* Collide b's edges against this geom's tri-mesh */
		if (contacts.size() < max_contacts) {
			transTo = m_invOrient * b->m_orient;
			b->CollideEdgesWithTrisOf2(contacts, max_contacts, this, transTo);
		}

		{
			PROFILE_SCOPED_DESC("Run Callbacks")
 ...
 * Generate collision contacts.
 */
void Geom::CollideEdgesWithTrisOf2(std::vector<CollisionContact> &contacts, int maxContacts, const Geom *b, const matrix4x4d &transTo) const
{
	PROFILE_SCOPED()
	struct stackobj {
		uint32_t edgeNode;
	...
	SingleBVHTreeBase *edgeBvh = GetGeomTree()->GetEdgeTree2();
	SingleBVHTreeBase *triBvh = b->GetGeomTree()->GetTriTree2();
...
void Geom::CollideEdgeTris(std::vector<CollisionContact> &contacts, const matrix4x4d &transToB, const Geom *b, uint32_t edgeIdx, uint32_t triNode, std::vector<uint32_t> &isect_buf) const
{
	...
	isect_buf.clear();
	b->GetGeomTree()->GetTriTree2()->TraceRay(v1, 1.0 / dir, edge.len, isect_buf, triNode);
