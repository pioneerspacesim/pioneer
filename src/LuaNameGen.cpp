...
std::string LuaNameGen::FullName(bool isFemale, RefCountedPtr<MTRand> &rng)
{
	...
	LuaObject<MTRand>::PushToLua(rng.Get());
...
std::string LuaNameGen::Surname(RefCountedPtr<MTRand> &rng)
{
	lua_State *l = m_luaManager->GetLuaState();
...
std::string LuaNameGen::BodyName(SystemBody *body, RefCountedPtr<MTRand> &rng)
{
	lua_State *l = m_luaManager->GetLuaState();
