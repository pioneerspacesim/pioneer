...
void ExternalCameraController::ZoomOut(float frameTime)
{
	...
	m_dist = FastMax(GetShip()->GetClipRadius(), m_dist);
	m_distTo = m_dist;
}
...
{
	m_distTo += 400*amount;
	m_distTo = FastMax(GetShip()->GetClipRadius(), m_distTo);
}
...
{
	AnimationCurves::Approach(m_dist, m_distTo, frameTime, 4.0, 50./FastMax(m_distTo, 1e-7));		// FastMax() here just avoid dividing by 0.
	m_dist = FastMax(GetShip()->GetClipRadius(), m_dist);
}
