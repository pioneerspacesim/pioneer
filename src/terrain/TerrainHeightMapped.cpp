...
double TerrainHeightFractal<TerrainHeightMapped>::GetHeight(const vector3d &p) const
{
...

	{
		...
		double mountains = h * h * 0.001 * octavenoise(GetFracDef(3 - m_fracnum), 0.5 * octavenoise(GetFracDef(5 - m_fracnum), 0.45, p), p) * ridged_octavenoise(GetFracDef(4 - m_fracnum), 0.475 * octavenoise(GetFracDef(6 - m_fracnum), 0.4, p), p);
		v += mountains;
		...
		if (v < 10.0) {
			v += 2.0 * v * dunes_octavenoise(GetFracDef(6 - m_fracnum), 0.5, p) * octavenoise(GetFracDef(6 - m_fracnum), 0.5, p);
		} else if (v < 50.0) {
			v += 20.0 * dunes_octavenoise(GetFracDef(6 - m_fracnum), 0.5, p) * octavenoise(GetFracDef(6 - m_fracnum), 0.5, p);
		} else {
			v += (50.0 / v) * (50.0 / v) * (50.0 / v) * (50.0 / v) * (50.0 / v) * 20.0 * dunes_octavenoise(GetFracDef(6 - m_fracnum), 0.5, p) * octavenoise(GetFracDef(6 - m_fracnum), 0.5, p);
		}
		if (v < 40.0) {
			//v = v;
		} else if (v < 60.0) {
		...
		v += h * 0.2 * voronoiscam_octavenoise(GetFracDef(5 - m_fracnum), Clamp(1.0 - (h * 0.0002), 0.0, 0.6), p) * Clamp(1.0 - (h * 0.0006), 0.0, 1.0);
