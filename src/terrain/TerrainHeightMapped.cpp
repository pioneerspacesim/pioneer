...
template <>
TerrainHeightFractal<TerrainHeightMapped>::TerrainHeightFractal(const SystemBody *body, const Uint32 surfaceEffects, const ETerrainColours terrainColour) :
...
void TerrainHeightFractal<TerrainHeightMapped>::GetHeights(const vector3d *vP, double *heightsOut, const size_t count) const
{
	...
	for (size_t i = 0; i < count; i++) {
		...
		double mountains = h * h * 0.001 * octavenoise(m_fracdef[3], 0.5 * octavenoise(m_fracdef[5], 0.45, p), p) * ridged_octavenoise(m_fracdef[4], 0.475 * octavenoise(m_fracdef[6], 0.4, p), p);
		v += mountains;
		//smaller ridged mountains
		if (v < 50.0) {
			v += v * v * 0.04 * ridged_octavenoise(m_fracdef[5], 0.5, p);
		} else if (v < 100.0) {
			v += 100.0 * ridged_octavenoise(m_fracdef[5], 0.5, p);
		} else {
			v += (100.0 / v) * (100.0 / v) * (100.0 / v) * (100.0 / v) * (100.0 / v) *
				100.0 * ridged_octavenoise(m_fracdef[5], 0.5, p);
		}
		...
		if (v < 10.0) {
			v += 2.0 * v * dunes_octavenoise(m_fracdef[6], 0.5, p) * octavenoise(m_fracdef[6], 0.5, p);
		} else if (v < 50.0) {
			v += 20.0 * dunes_octavenoise(m_fracdef[6], 0.5, p) * octavenoise(m_fracdef[6], 0.5, p);
		} else {
			v += (50.0 / v) * (50.0 / v) * (50.0 / v) * (50.0 / v) * (50.0 / v) * 20.0 * dunes_octavenoise(m_fracdef[6], 0.5, p) * octavenoise(m_fracdef[6], 0.5, p);
		}
		if (v < 40.0) {
			//v = v;
		} else if (v < 60.0) {
			v += (v - 40.0) * billow_octavenoise(m_fracdef[5], 0.5, p);
			//Output("V/height: %f\n", Clamp(v-20.0, 0.0, 1.0));
		} else {
			v += (30.0 / v) * (30.0 / v) * (30.0 / v) * 20.0 * billow_octavenoise(m_fracdef[5], 0.5, p);
		}

		//ridges and bumps
		//v += h*0.1*ridged_octavenoise(GetFracDef(6-m_fracnum), Clamp(h*0.0002, 0.3, 0.5), p)
		//	* Clamp(h*0.0002, 0.1, 0.5);
		v += h * 0.2 * voronoiscam_octavenoise(m_fracdef[5], Clamp(1.0 - (h * 0.0002), 0.0, 0.6), p) * Clamp(1.0 - (h * 0.0006), 0.0, 1.0);
		//polar ice caps with cracks
		if ((m_icyness * 0.5) + (fabs(p.y * p.y * p.y * 0.38)) > 0.6) {
			h = Clamp(1.0 - (v * 10.0), 0.0, 1.0) * voronoiscam_octavenoise(m_fracdef[5], 0.5, p);
			h *= h * h * 2.0;
			h -= 3.0;
			v += h;
		}
