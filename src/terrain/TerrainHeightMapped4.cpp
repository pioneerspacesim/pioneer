...
template <>
const char *TerrainHeightFractal<TerrainHeightMapped4>::GetHeightFractalName() const { return "Mapped"; }

template <>
TerrainHeightFractal<TerrainHeightMapped4>::TerrainHeightFractal(const SBody *body) : Terrain(body)
{
}

template <>
double TerrainHeightFractal<TerrainHeightMapped4>::GetHeight(const vector3d &p)
{
...
//	}
	...
	double v;
	{
		...
		double a2 = 0.5*d0 + 0.5*d2;
		double a3 = -(1/6.0)*d0 - 0.5*d2 + (1/6.0)*d3;
		v = a0 + a1*dy + a2*dy*dy + a3*dy*dy*dy;
