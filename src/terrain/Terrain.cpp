...
Terrain::Terrain(const SystemBody *body) : m_body(body), m_seed(body->seed), m_rand(body->seed), m_heightScaling(0), m_minh(0) {
		...
		switch (m_body->heightMapFractal) {
			case 0: {
				...
				for(Uint32 i=0; i<heightmapPixelArea; i++) {
					...
					minHMap = Min(minHMap, val);
					maxHMap = Max(maxHMap, val);
			...
			case 1: {
				...
				for(Uint32 i=0; i<heightmapPixelArea; i++) {
					...
					minHMapScld = Min(minHMapScld, val);
					maxHMapScld = Max(maxHMapScld, val);
					// store then increment pointer
					(*pHeightMap) = val;
					++pHeightMap;
				}
	...
	if ((m_body->heightMapFilename) && m_body->heightMapFractal > 1){ // if scaled heightmap
		...
		m_maxHeightInMeters = Max(100.0, (9000.0*rad*rad*(m_volcanic+0.5)) / (m_body->GetMass() * 6.64e-12));
	...
	m_maxHeight = Min(1.0, m_maxHeightInMeters / rad);
	...
	for (int i=0; i<int(COUNTOF(m_rockColor)); i++) {
		...
		r = Max(b, r * m_body->m_metallicity.ToFloat());
		g = Max(b, g * m_body->m_metallicity.ToFloat());
	...
	for (int i=0; i<int(COUNTOF(m_plantColor)); i++) {
		...
		g = Max(r, g * m_body->m_life.ToFloat());
...
void Terrain::SetFracDef(const unsigned int index, const double featureHeightMeters, const double featureWidthMeters, const double smallestOctaveMeters)
{
	...
	m_fracdef[index].octaves = Max(1, int(ceil(log(featureWidthMeters / smallestOctaveMeters) / log(2.0))));
