...
Terrain::Terrain(const SystemBody *body) : m_seed(body->seed), m_rand(body->seed), m_heightScaling(0), m_minh(0), m_minBody(body) {
	...
	if ((body->heightMapFilename) && body->heightMapFractal > 1){ // if scaled heightmap
		...
		m_maxHeightInMeters = Max(100.0, (9000.0*rad*rad*(m_volcanic+0.5)) / (body->GetMass() * 6.64e-12));
	...
	for (int i=0; i<int(COUNTOF(m_rockColor)); i++) {
		...
		r = Max(b, r * body->m_metallicity.ToFloat());
		g = Max(b, g * body->m_metallicity.ToFloat());
