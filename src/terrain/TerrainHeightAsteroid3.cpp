...
template <>
TerrainHeightFractal<TerrainHeightAsteroid3>::TerrainHeightFractal(const SystemBody *body, const Uint32 surfaceEffects, const ETerrainColours terrainColour) :
...
void TerrainHeightFractal<TerrainHeightAsteroid3>::GetHeights(const vector3d *vP, double *heightsOut, const size_t count) const
{
	for (size_t i = 0; i < count; i++) {
		...
		const double n = octavenoise(m_fracdef[0], 0.5, p) * ridged_octavenoise(m_fracdef[1], 0.5, p) * m_maxHeight;
