...
template <>
void TerrainColorFractal<TerrainColorDesert>::GetColor(const vector3d &p, const double height, const vector3d &norm, Color3ub &out) const
{
	...
	if (fabs(m_icyness*p.y) + m_icyness*n > 1) {
		SetColour(out, interpolate_color(flatness, color_cliffs, vector3d(1,1,1))); return;
	}
