...
template <>
TerrainHeightFractal<TerrainHeightMountainsNormal>::TerrainHeightFractal(const SystemBody *body, const Uint32 surfaceEffects, const ETerrainColours terrainColour) :
	Terrain(body, surfaceEffects, terrainColour)
{
	SetFracDef(0, m_maxHeightInMeters, m_rand.Double(1e6, 1e7), 10000);
	SetFracDef(1, m_maxHeightInMeters * 0.00000000001, 100.0, 10);
	SetFracDef(2, m_maxHeightInMeters * 0.0000001, m_rand.Double(500, 2e3), 1000);
	SetFracDef(3, m_maxHeightInMeters * 0.00002, m_rand.Double(1500, 1e4), 100);
	SetFracDef(4, m_maxHeightInMeters * 0.08, 1e4, 100);
	SetFracDef(5, m_maxHeightInMeters * 0.2, 1e5, 100);
	SetFracDef(6, m_maxHeightInMeters * 0.5, 1e6, 1000);
	SetFracDef(7, m_maxHeightInMeters * 0.5, m_rand.Double(1e6, 1e7), 1000);
	SetFracDef(8, m_maxHeightInMeters, m_rand.Double(3e6, 1e7), 1000);
}
...
void TerrainHeightFractal<TerrainHeightMountainsNormal>::GetHeights(const vector3d *vP, double *heightsOut, const size_t count) const
{
	for (size_t i = 0; i < count; i++) {
		...
		if (n > 0.0) {
			...
			n += h * 0.2 * ridged_octavenoise(m_fracdef[7], 0.5 * octavenoise(m_fracdef[6], 0.5, p), p);

			// This smoothes edges near the coast, we cant have vertical terrain its not handled correctly.
			if (n < 0.4) {
				n += n * 1.25 * ridged_octavenoise(m_fracdef[6], Clamp(h * 0.00002, 0.3, 0.7) * ridged_octavenoise(m_fracdef[5], 0.5, p), p);
			} else {
				n += 0.5 * ridged_octavenoise(m_fracdef[6], Clamp(h * 0.00002, 0.3, 0.7) * ridged_octavenoise(m_fracdef[5], 0.5, p), p);
			}

			if (n < 0.2) {
				n += n * 15.0 * river_octavenoise(m_fracdef[6], Clamp(h * 0.00002, 0.5, 0.7), p);
			} else {
				n += 3.0 * river_octavenoise(m_fracdef[6], Clamp(h * 0.00002, 0.5, 0.7), p);
			}
			n *= 0.33333333333;
			...
			if (n > 0.25) {
				n += (n - 0.25) * 0.1 * octavenoise(m_fracdef[3], Clamp(h * 0.0002 * octavenoise(m_fracdef[5], 0.5, p), 0.5 * octavenoise(m_fracdef[3], 0.5, p), 0.5 * octavenoise(m_fracdef[3], 0.5, p)), p); //[4]?
			}
