...
template <>
void TerrainColorFractal<TerrainColorGGUranus>::GetColor(const vector3d &p, const double height, const vector3d &norm, Color3ub &out) const
{
	...
	n /= 2.0;
	n *= n*n;
	SetColour(out, interpolate_color(n, vector3d(.4, .5, .55), vector3d(.85,.95,.96)));
}
