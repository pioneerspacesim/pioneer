...
{
	double n = 0.8 * octavenoise(m_fracdef[2], 0.6, vector3d(3.142 * p.y * p.y));
	n += 0.25 * ridged_octavenoise(m_fracdef[3], 0.55, vector3d(3.142 * p.y * p.y));
	n += 0.2 * octavenoise(m_fracdef[3], 0.5, vector3d(3.142 * p.y * p.y));
	//spot
	n += 0.8 * billow_octavenoise(m_fracdef[1], 0.8, vector3d(noise(p * 3.142) * p)) *
