...
namespace {
	...
	static RefCountedPtr<Graphics::Material> GetMaterial()
	{
		return s_matShield;
	}
};
	...
	: m_enabled(false)
{
	...
	SceneGraph::FindNodeVisitor shieldFinder(SceneGraph::FindNodeVisitor::MATCH_NAME_FULL, "shield");
	...
	for (unsigned int i=0; i < results.size(); i++) {
		...
		for(Uint32 iChild=0 ; iChild<mt->GetNumChildren() ; ++iChild) {
			...
			for (Uint32 iMesh=0; iMesh<sg->GetNumMeshes(); ++iMesh) {
				...
				rMesh->GetSurface(0)->SetMaterial(GetMaterial());
			}
