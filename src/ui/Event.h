...
namespace UI {
	...
	public:
		...
		KeyboardEvent(Action _action, const KeySym &_keysym, bool _repeat) :
	...
	protected:
		MouseEvent(Event::Type _type, const Point &_pos) :
			Event(_type),
			pos(_pos) {}
	};
	...
	public:
		...
		MouseButtonEvent(Action _action, ButtonType _button, const Point &_pos) :
			MouseEvent(Event::MOUSE_BUTTON, _pos),
			action(_action),
			button(_button) {}
		const Action action;
	...
	public:
		MouseMotionEvent(const Point &_pos, const Point &_rel) :
			MouseEvent(Event::MOUSE_MOTION, _pos),
			rel(_rel) {}
		const Point rel;
	...
	public:
		...
		MouseWheelEvent(WheelDirection _direction, const Point &_pos) :
			MouseEvent(Event::MOUSE_WHEEL, _pos),
	...
	protected:
		JoystickEvent(Event::Type _type, SDL_JoystickID _joystick) :
			Event(_type),
			joystick(_joystick) {}
	};
	...
	public:
		...
		JoystickAxisMotionEvent(SDL_JoystickID _joystick, float _value, int _axis) :
			JoystickEvent(Event::JOYSTICK_AXIS_MOTION, _joystick),
			value(_value),
			axis(_axis) {}
	...
	public:
		...
		JoystickHatMotionEvent(SDL_JoystickID _joystick, JoystickHatDirection _direction, int _hat) :
			JoystickEvent(Event::JOYSTICK_HAT_MOTION, _joystick),
			direction(_direction),
			hat(_hat) {}
	...
	public:
		...
		JoystickButtonEvent(SDL_JoystickID _joystick, Action _action, int _button) :
			JoystickEvent(Event::JOYSTICK_BUTTON, _joystick),
			action(_action),
			button(_button) {}
