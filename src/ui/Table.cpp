...
void Table::LayoutAccumulator::AddRow(const std::vector<Widget*> &widgets)
{
	...
	for (std::size_t i = 0; i < widgets.size(); i++) {
		...
		// XXX handle flags
		m_columnWidth[i] = FastMax(m_columnWidth[i], size.x);
...
Point Table::Inner::PreferredSize()
{
	...
	for (std::size_t i = 0; i < m_rows.size(); i++) {
		...
		for (std::size_t j = 0; j < row.size(); j++) {
			...
			m_rowHeight[i] = FastMax(m_rowHeight[i], size.y);
		}
...
void Table::Inner::Layout()
{
	...
	for (std::size_t i = 0; i < m_rows.size(); i++) {
		...
		for (std::size_t j = 0; j < row.size(); j++) {
			...
			int height = FastMin(preferredSize.y, m_rowHeight[i]);
...
Point Table::PreferredSize()
{
	...
	return Point(FastMax(headerPreferredSize.x,bodyPreferredSize.x)+sliderSize.x, headerPreferredSize.y+bodyPreferredSize.y);
}
