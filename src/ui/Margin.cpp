...
void Margin::Layout()
{
	...
	switch (m_direction) {
		case ALL:
			...
			innerSize = Point(FastMax(size.x-m_margin*2,0), FastMax(size.y-m_margin*2,0));
			break;

		case HORIZONTAL:
			innerPos = Point(m_margin,0);
			innerSize = Point(FastMax(size.x-m_margin*2,0), size.y);
			break;

		case VERTICAL:
			innerPos = Point(0,m_margin);
			innerSize = Point(size.x, FastMax(size.y-m_margin*2,0));
			break;

		case LEFT:
			innerPos = Point(m_margin,0);
			innerSize = Point(FastMax(size.x-m_margin,0), size.y);
			break;
		...
		case TOP:
			innerPos = Point(0,m_margin);
			innerSize = Point(size.x, FastMax(size.y-m_margin,0));
			break;
