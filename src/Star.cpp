...
void Star::InitStar()
{
	...
	const float wf = (sbody->m_type < SystemBody::TYPE_STAR_S_BH && sbody->m_type > SystemBody::TYPE_STAR_O_HYPER_GIANT) ? 100.0f : 1.0f;
...
void Star::Render(Graphics::Renderer *renderer, const Camera *camera, const vector3d &viewCoords, const matrix4x4d &viewTransform)
{
	...
	const Uint8 *col = StarSystem::starRealColors[GetSystemBody()->m_type];
