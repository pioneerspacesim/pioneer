	...
	m_showHyperspaceButton(false)
{
	...
	m_wheelsButton->AddState(0, PIONEER_DATA_DIR "/icons/wheels_up.png", PiLang::WHEELS_ARE_UP);
	m_wheelsButton->AddState(1, PIONEER_DATA_DIR "/icons/wheels_down.png", PiLang::WHEELS_ARE_DOWN);
	...
	labels_button->AddState(1, PIONEER_DATA_DIR "/icons/labels_on.png", PiLang::OBJECT_LABELS_ARE_ON);
	labels_button->AddState(0, PIONEER_DATA_DIR "/icons/labels_off.png", PiLang::OBJECT_LABELS_ARE_OFF);
	...
	m_hyperspaceButton->SetToolTip(PiLang::HYPERSPACE_JUMP);
	...
	m_launchButton->SetToolTip(PiLang::TAKEOFF);
	...
	m_flightControlButton->AddState(Player::CONTROL_MANUAL, PIONEER_DATA_DIR "/icons/manual_control.png", PiLang::MANUAL_CONTROL);
	m_flightControlButton->AddState(Player::CONTROL_FIXSPEED, PIONEER_DATA_DIR "/icons/manual_control.png", PiLang::COMPUTER_SPEED_CONTROL);
	m_flightControlButton->AddState(Player::CONTROL_AUTOPILOT, PIONEER_DATA_DIR "/icons/autopilot.png", PiLang::AUTOPILOT_ON);
	...
	m_hudVelocity->SetToolTip(PiLang::SHIP_VELOCITY_BY_REFERENCE_OBJECT);
	m_hudTargetDist->SetToolTip(PiLang::DISTANCE_FROM_SHIP_TO_NAV_TARGET);
	m_hudAltitude->SetToolTip(PiLang::SHIP_ALTITUDE_ABOVE_TERRAIN);
	m_hudPressure->SetToolTip(PiLang::EXTERNAL_ATMOSPHERIC_PRESSURE);
	...
	m_hudHullTemp = new Gui::MeterBar(100.0f, PiLang::HULL_TEMP, Color(1.0f,0.0f,0.0f,0.8f));
	m_hudWeaponTemp = new Gui::MeterBar(100.0f, PiLang::WEAPON_TEMP, Color(1.0f,0.5f,0.0f,0.8f));
	m_hudHullIntegrity = new Gui::MeterBar(100.0f, PiLang::HULL_INTEGRITY, Color(1.0f,1.0f,0.0f,0.8f));
	m_hudShieldIntegrity = new Gui::MeterBar(100.0f, PiLang::SHIELD_INTEGRITY, Color(1.0f,1.0f,0.0f,0.8f));
	...
	m_hudTargetHullIntegrity = new Gui::MeterBar(100.0f, PiLang::HULL_INTEGRITY, Color(1.0f,1.0f,0.0f,0.8f));
	m_hudTargetShieldIntegrity = new Gui::MeterBar(100.0f, PiLang::SHIELD_INTEGRITY, Color(1.0f,1.0f,0.0f,0.8f));
	...
	if (Pi::player->GetFlightState() == Ship::DOCKED) {
		if (!Pi::player->Undock()) {
			Pi::cpan->MsgLog()->ImportantMessage(Pi::player->GetDockedWith()->GetLabel(),
					PiLang::LAUNCH_PERMISSION_DENIED_BUSY);
		}
	} else {
		Pi::player->Blastoff();
	}
...
void WorldView::OnClickHyperspace()
{
	if (Pi::player->GetHyperspaceCountdown() > 0.0) {
		...
		Pi::cpan->MsgLog()->Message("", PiLang::HYPERSPACE_JUMP_ABORTED);
	} else {
...
void WorldView::Draw3D()
{
   ...
   const float FOV_MIN = 20.0f;  // Minimum FOV in degrees
	...
	Frame cam_frame(Pi::player->GetFrame(), PiLang::CAMERA, Frame::TEMP_VIEWING);
...
void WorldView::RefreshButtonStateAndVisibility()
{
	if ((!Pi::player) || Pi::player->IsDead() || !Pi::IsGameStarted()) {
		...
		switch(Pi::player->GetFlightState()) {
			case Ship::LANDED:
				m_flightStatus->SetText(PiLang::LANDED);
				m_launchButton->Show();
				m_flightControlButton->Hide();
				break;
				
			case Ship::DOCKING:
				m_flightStatus->SetText(PiLang::DOCKING);
				m_launchButton->Hide();
				m_flightControlButton->Hide();
				break;

			case Ship::DOCKED:
				m_flightStatus->SetText(PiLang::DOCKED);
				m_launchButton->Show();
				m_flightControlButton->Hide();
				break;

			case Ship::HYPERSPACE:
				m_flightStatus->SetText(PiLang::HYPERSPACE);
				m_launchButton->Hide();
				m_flightControlButton->Hide();
				break;
	...
	if (const SystemPath *dest = Space::GetHyperspaceDest()) {
		...
		if (Body *navtarget = Pi::player->GetNavTarget()) {
			...
			char buf[128];
			snprintf(buf, sizeof(buf), PiLang::N_DISTANCE_TO_TARGET, format_distance(dist).c_str());
		...
		if (Pi::player->GetFrame()->m_astroBody) {
			...
			if (altitude > 9999999.0 || astro->IsType(Object::SPACESTATION)) {
				...
				char buf[128];
				snprintf(buf, sizeof(buf), PiLang::ALT_IN_METRES, altitude);
			...
			if (astro->IsType(Object::PLANET)) {
				...
				char buf[128];
				snprintf(buf, sizeof(buf), PiLang::PRESSURE_N_BAR, pressure);
	...
	if (b) {
		if (b->IsType(Object::SHIP) && Pi::player->m_equipment.Get(Equip::SLOT_RADARMAPPER) == Equip::RADAR_MAPPER) {
			...
			if (s->m_equipment.Get(Equip::SLOT_ENGINE) == Equip::NONE) {
				text += PiLang::NO_HYPERDRIVE;
			} else {
				text += EquipType::types[s->m_equipment.Get(Equip::SLOT_ENGINE)].name;
			}

			text += stringf(256, std::string(std::string("\n")+std::string(PiLang::MASS_N_TONNES)+std::string("\n")).c_str(), stats->total_mass);
			text += stringf(256, std::string(std::string(PiLang::SHIELD_STRENGTH_N)+std::string("\n")).c_str(),
				(sShields*0.01f) * float(s->m_equipment.Count(Equip::SLOT_CARGO, Equip::SHIELD_GENERATOR)));
			text += stringf(256, std::string(std::string(PiLang::CARGO_N)+std::string("\n")).c_str(), stats->used_cargo);
			...
			if (!ship) {
				text += PiLang::HYPERSPACE_ARRIVAL_CLOUD_REMNANT;
			}
			else {
				...
				text += stringf(512,
					std::string(
                        std::string(PiLang::HYPERSPACE_X_CLOUD)+std::string("\n")+
					    std::string(PiLang::SHIP_MASS_N_TONNES)+std::string("\n")+
					    std::string("%s: %s\n")+
					    std::string(PiLang::DATE_DUE_N)+std::string("\n")
                    ).c_str(),
					cloud->IsArrival() ? PiLang::ARRIVAL : PiLang::DEPARTURE,
					ship->CalcStats()->total_mass,
                    cloud->IsArrival() ? PiLang::SOURCE : PiLang::DESTINATION,
	...
	if (Pi::player->GetHyperspaceCountdown() != 0) {
		...
		if (!(int(ceil(val*2.0)) % 2)) {
			char buf[128];
			snprintf(buf, sizeof(buf), PiLang::HYPERSPACING_IN_N_SECONDS, ceil(val));
...
void WorldView::BuildCommsNavOptions()
{
	...
	m_commsNavOptions->PackEnd(new Gui::Label(std::string("#ff0")+std::string(PiLang::NAVIGATION_TARGETS_IN_THIS_SYSTEM)+std::string("\n")));
...
static void PlayerPayFine()
{
	...
	if (Pi::player->GetMoney() == 0) {
		Pi::cpan->MsgLog()->Message("", PiLang::YOU_NO_MONEY);
	} else if (fine > Pi::player->GetMoney()) {
		Polit::AddCrime(0, -Pi::player->GetMoney());
		Polit::GetCrime(&crime, &fine);
		Pi::cpan->MsgLog()->Message("", stringf(512, PiLang::FINE_PAID_N_BUT_N_REMAINING,
		...
		Pi::cpan->MsgLog()->Message("", stringf(512, PiLang::FINE_PAID_N,
				format_money(fine).c_str()));
...
void WorldView::OnHyperspaceTargetChanged()
{
	...
	Pi::cpan->MsgLog()->Message("", std::string(PiLang::SET_HYPERSPACE_DESTINATION_TO+system->GetName()));
	system->Release();
...
void WorldView::UpdateCommsOptions()
{
	...
	if (navtarget) {
		...
		if (navtarget->IsType(Object::SPACESTATION)) {
			button = AddCommsOption(PiLang::REQUEST_DOCKING_CLEARANCE, ypos, optnum++);
			...
			if (Pi::player->m_equipment.Get(Equip::SLOT_AUTOPILOT) == Equip::AUTOPILOT) {
				button = AddCommsOption(PiLang::AUTOPILOT_DOCK_WITH_STATION, ypos, optnum++);
			...
			Polit::GetCrime(&crime, &fine);
			if (fine) {
				button = AddCommsOption(stringf(512, PiLang::PAY_FINE_REMOTELY,
		...
		if (Pi::player->m_equipment.Get(Equip::SLOT_AUTOPILOT) == Equip::AUTOPILOT) {
			button = AddCommsOption(PiLang::AUTOPILOT_FLY_TO_VICINITY_OF + navtarget->GetLabel(), ypos, optnum++);
			...
			if (navtarget->IsType(Object::PLANET) || navtarget->IsType(Object::STAR)) {
				button = AddCommsOption(PiLang::AUTOPILOT_ENTER_LOW_ORBIT_AROUND + navtarget->GetLabel(), ypos, optnum++);
				button->onClick.connect(sigc::bind(sigc::ptr_fun(autopilot_orbit), navtarget, 1.1));
				ypos += 32;

				button = AddCommsOption(PiLang::AUTOPILOT_ENTER_MEDIUM_ORBIT_AROUND + navtarget->GetLabel(), ypos, optnum++);
				button->onClick.connect(sigc::bind(sigc::ptr_fun(autopilot_orbit), navtarget, 2.0));
				ypos += 32;

				button = AddCommsOption(PiLang::AUTOPILOT_ENTER_HIGH_ORBIT_AROUND + navtarget->GetLabel(), ypos, optnum++);
		...
		if ((t != Equip::NONE) && navtarget->IsType(Object::HYPERSPACECLOUD)) {
			...
			if (!cloud->IsArrival()) {
				button = AddCommsOption(PiLang::SET_HYPERSPACE_TARGET_TO_FOLLOW_THIS_DEPARTURE, ypos, optnum++);
		...
		if (b) {
			...
			std::string msg = PiLang::SET_HYPERSPACE_TARGET_TO + navtarget->GetLabel();
	...
	if (comtarget) {
		...
		ypos += 32;
		button = AddCommsOption(PiLang::AUTOPILOT_FLY_TO_VICINITY_OF+comtarget->GetLabel(), ypos, optnum++);
