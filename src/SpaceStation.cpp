...
void SpaceStation::InitStation()
{
	...
	Random rand(m_sbody->m_seed);
	bool ground = m_sbody->m_type == SystemBody::TYPE_STARPORT_ORBITAL ? false : true;
	if (ground) {
...
void SpaceStation::Render(Graphics::Renderer *r, const Camera *camera, const vector3d &viewCoords, const matrix4x4d &viewTransform)
{
	...
	if (!b->IsType(Object::PLANET)) {
		...
		if (!m_adjacentCity) {
			m_adjacentCity = new CityOnPlanet(planet, this, m_sbody->m_seed);
		}
