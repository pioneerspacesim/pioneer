	...
	if (Pi::player->GetFlightState() != Ship::DOCKED
		...
		if (port != -1) {
			...
			{ // blue and white thang
				...
				strncpy(f.regid, PiLang::POLICE_SHIP_REGISTRATION, 16);
...
bool SpaceStation::GetDockingClearance(Ship *s, std::string &outMsg)
{
	for (int i=0; i<MAX_DOCKING_PORTS; i++) {
		...
		if ((m_shipDocking[i].ship == s) && (m_shipDocking[i].stage > 0)) {
			outMsg = stringf(256, PiLang::CLEARANCE_ALREADY_GRANTED_BAY_N, i+1);
			return true;
		}
	}
	for (int i=0; i<MAX_DOCKING_PORTS; i++) {
		...
		sd.stage = 1;
		sd.stagePos = 0;
		outMsg = stringf(256, PiLang::CLEARANCE_GRANTED_BAY_N, i+1);
		return true;
	}
	outMsg = PiLang::CLEARANCE_DENIED_NO_BAYS;
	return false;
}
...
bool SpaceStation::CanSell(Equip::Type t, bool verbose) const {
	...
	if (verbose && !result) {
		Pi::Message(PiLang::ITEM_IS_OUT_OF_STOCK);
	}
	return result;
}
