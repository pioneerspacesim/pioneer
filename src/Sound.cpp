...
public:
	...
	size_t read(char *buf, size_t sz, int n)
	{
		...
		n = FastMin(n, int((m_data->GetSize() - offset) / sz));
	...
	while ((pos < len) && ev.sample) {
		...
		while (pos < len) {
			...
			for (int chan=0; chan<2; chan++) {
				if (ev.ascend[chan]) {
					ev.volume[chan] = FastMin(ev.volume[chan] + ev.rateOfChange[chan], ev.targetVolume[chan]);
				} else {
					ev.volume[chan] = FastMax(ev.volume[chan] - ev.rateOfChange[chan], ev.targetVolume[chan]);
				}
			}
