...
void NotifyOfCrime(Ship *s, enum Crime crime)
{
	...
	SpaceStation *station = static_cast<SpaceStation*>(Pi::spaceManager->GetCurrentSpace()->FindNearestTo(s, Object::SPACESTATION));
	if (station) {
		...
		float lawlessness = Pi::spaceManager->GetCurrentSpace()->GetStarSystem()->GetSysPolit().lawlessness.ToFloat();
...
{
	int politType = Pi::spaceManager->GetCurrentSpace()->GetStarSystem()->GetSysPolit().govType;
