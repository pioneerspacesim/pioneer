...
void NotifyOfCrime(Ship *s, enum Crime crime)
{
	...
	SpaceStation *station = static_cast<SpaceStation*>(Pi::spaceManager->GetSpace()->FindNearestTo(s, Object::SPACESTATION));
	if (station) {
		...
		float lawlessness = Pi::spaceManager->GetSpace()->GetStarSystem()->GetSysPolit().lawlessness.ToFloat();
...
{
	int politType = Pi::spaceManager->GetSpace()->GetStarSystem()->GetSysPolit().govType;
