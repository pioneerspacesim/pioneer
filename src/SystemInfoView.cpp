...
#define _add_label_and_value(label, value) { \
	...
	_add_label_and_value(Lang::MASS, stringf_old(64, Lang::N_WHATEVER_MASSES, b->mass.ToDouble(), 
		(b->GetSuperType() == SBody::SUPERTYPE_STAR ? Lang::SOLAR : Lang::EARTH)));

	_add_label_and_value(Lang::SURFACE_TEMPERATURE, stringf_old(64, Lang::N_CELSIUS, b->averageTemp-273));

	if (b->parent) {
		float days = float(b->orbit.period) / float(60*60*24);
		if (days > 1000) {
			data = stringf_old(64, Lang::N_YEARS, days/365);
		} else {
			data = stringf_old(64, Lang::N_DAYS, b->orbit.period / (60*60*24));
		}
		...
		_add_label_and_value(Lang::AXIAL_TILE, stringf_old(64, Lang::N_DEGREES, b->axialTilt.ToDouble() * (180.0/M_PI) ));
		if (b->rotationPeriod != 0) {
			_add_label_and_value(
				std::string(Lang::DAY_LENGTH)+std::string(Lang::ROTATIONAL_PERIOD),
				stringf_old(64, Lang::N_EARTH_DAYS, b->rotationPeriod.ToDouble()));
		}
...
void SystemInfoView::SystemChanged(StarSystem *s)
{
...

	{
		...
		if (pop >= fixed(1,1)) { popmsg = stringf_old(256, Lang::OVER_N_BILLION, pop.ToInt32()); }
		else if (pop >= fixed(1,1000)) { popmsg = stringf_old(256, Lang::OVER_N_MILLION, (pop*1000).ToInt32()); }
