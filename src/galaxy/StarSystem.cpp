...
SystemBody::AtmosphereParameters SystemBody::CalcAtmosphereParams() const
{
	...
	params.atmosInvScaleHeight = FastMax(20.0f, static_cast<float>(GetRadius() / atmosScaleHeight));
...
{
	PROFILE_SCOPED()
	discMax = FastMax(discMax, discMin);
...
void StarSystem::MakePlanetsAround(SystemBody *primary, Random &rand)
{
	...
	if (superType <= SystemBody::SUPERTYPE_STAR) {
		...
		if ((superType == SystemBody::SUPERTYPE_STAR) && (primary->parent)) {
			...
			discMax = FastMin(discMax, primary->orbMin * fixed(1,10));
		}

		/* in trinary and quaternary systems don't bump into other pair... */
		if (m_numStars >= 3) {
			discMax = FastMin(discMax, fixed(5,100)*rootBody->children[0]->orbMin);
		}
	} else {
		...
		discMax = FastMin(discMax, fixed(1,20)*
...
void SystemBody::PickPlanetType(Random &rand)
{
	...
	radius = FastMax(radius, fixed(1,630));
	...
	m_volcanicity = FastMin(fixed(1,1), mass) * rand.Fixed();
	...
	if (mass > 317*13) {
		...
		mass = FastMin(mass, fixed(317*65, 1)) / SUN_MASS_TO_EARTH_MASS;
...
//	Output("Trading rates:\n");
	...
	for (int i=Equip::FIRST_COMMODITY; i<=Equip::LAST_COMMODITY; i++) {
		maximum = FastMax(abs(m_tradeLevel[i]), maximum);
	}
...
void SystemBody::PopulateAddStations(StarSystem *system)
{
	...
	if (children.size()) orbMaxS = FastMin(orbMaxS, fixed(1,2) * children[0]->orbMin);
