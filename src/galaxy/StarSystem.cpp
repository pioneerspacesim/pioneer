...
void StarSystem::ExportToLua(const char *filename) {
	...
	for(i = 0; ENUM_BodyType[i].name != 0; i++) {
		if(ENUM_BodyType[i].value == rootBody->type)
			break;
	}
	...
	fprintf(f,"local system = CustomSystem:new('%s', { '%s' })\n\t:govtype('%s')\n\t:short_desc('%s')\n\t:long_desc([[%s]])",
			GetName().c_str(), ENUM_BodyType[i].name, ENUM_PolitGovType[j].name, GetShortDescription(), GetLongDescription());
	...
	*/
	fprintf(f, "system:add_to_sector(%d,%d,%d,v(%f,%f,%f))",
			GetPath().sectorX, GetPath().sectorY, GetPath().sectorZ,
			this->GetPath().pos);
}
