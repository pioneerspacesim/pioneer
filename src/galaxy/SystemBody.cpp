...
void SystemBody::CalcAtmosphereParams(AtmosphereParameters &params) const
{
	...
	for (int i = 0; i <= DENSITY_STEPS; ++i) {
		...
		for (int j = 0; j <= DENSITY_STEPS; ++j) {
			...
			// Parameterize the X coordinate as the linear angle between
			// vertical and horizon.
			//
			// The X coordinate could be raw cos(theta), but that puts most of
			// the precision near the zenith rather evenly distributed.
			float cos_theta = cos(M_PI * float(j) / DENSITY_STEPS);
			// dot(a, b) = a1*b1 + a2*b2 = b2
			vector2d direction = vector2d(sqrt(1 - cos_theta * cos_theta), cos_theta);
			...
			for (int k = 0; k < numSamples; ++k) {
				...
				double sampleHeight = std::max(samplePosition.Length() - radiusPlanet_in_m, 1.0);
			...
			rLogDensity = rDensity > 0.0 ? log(rDensity) : 0;
			mLogDensity = mDensity > 0.0 ? log(mDensity) : 0;
