...
void test_random()
{
	...
	for (int i = 0; i < 5; ++i)
		cout << i + 1 << ": " << (rnd.Int32() - in32out32[i] == 0 ? "pass" : "fail") << endl;

	cout << "32-bit seeds, Double out: " << endl;
	for (int i = 0; i < 5; ++i)
		cout << i + 1 << ": " << (fabs(rnd.Double() - in32outDbl[i]) < 0.000001 ? "pass" : "fail") << endl; // close enough
	...
	for (int i = 0; i < 5; ++i)
		cout << i + 1 << ": " << (rnd.Int32() - in64out32[i] == 0 ? "pass" : "fail") << endl;

	cout << "64-bit seeds, Double out: " << endl;
	for (int i = 0; i < 5; ++i)
		cout << i + 1 << ": " << (fabs(rnd.Double() - in64outDbl[i]) < 0.000001 ? "pass" : "fail") << endl; // close enough
