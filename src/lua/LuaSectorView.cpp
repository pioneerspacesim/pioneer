...
void LuaObject<SectorView>::RegisterClass()
{
	...
	metaType.StartRecording()
		...
		.AddFunction("SetSelected", &SectorView::SetSelected)
		.AddFunction("GotoSectorPath", &SectorView::GotoSector)
		.AddFunction("GotoSystemPath", &SectorView::GotoSystem)
		.AddFunction("GetFactions",
			[](lua_State *l, SectorView *sv) -> int {
		...
		.AddFunction("GetCenterSector",
			[](lua_State *l, SectorView *sv) -> int {
				LuaPush<vector3d>(l, vector3d(sv->GetCenterSector()));
				return 1;
			}
		)
		.AddFunction("MoveRouteItemUp",
			[](lua_State *l, SectorView *sv) -> int {
		...
		.AddFunction("SearchNearbyStarSystemsByName",
			[](lua_State *l, SectorView *sv) -> int {
		...
		.AddFunction("AutoRoute",
			[](lua_State *l, SectorView *sv) {
