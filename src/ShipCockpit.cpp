...
void ShipCockpit::Update(float timeStep)
{
	...
	float gforce = Clamp(floorf(((std::abs(cur_vel) - m_shipVel) / timeStep) / 9.8f), -COCKPIT_MAX_GFORCE, COCKPIT_MAX_GFORCE);
	if (std::abs(cur_vel) > 500000.0f ||      // Limit gforce measurement so we don't get astronomical fluctuations
		std::abs(gforce - m_gForce) > 100.0) { // Smooth out gforce one frame spikes, sometimes happens when hitting max speed due to the thrust limiters
		gforce = 0.0f;
	}
	if (std::abs(gforce - m_gForce) > 100.0) {
		gforce = 0.0f;
	}
	if (std::abs(m_translate.z - m_offset) < 0.001f) {
		m_transInterp = 0.0f;
	}
