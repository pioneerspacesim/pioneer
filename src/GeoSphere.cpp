...
static void print_info(const SystemBody *sbody, const Terrain *terrain)
{
	Output(
		...
		"    colour fractal: %s\n"
		"    seed: %u\n",
		sbody->m_name.c_str(), terrain->GetHeightFractalName(), terrain->GetColorFractalName(), sbody->m_seed);
}
...
void GeoSphere::Render(Graphics::Renderer *renderer, const matrix4x4d &modelView, vector3d campos, const float radius, const float scale, const std::vector<Camera::Shadow> &shadows)
{
	...
	if ((m_sbody->GetSuperType() == SystemBody::SUPERTYPE_STAR) || (m_sbody->m_type == SystemBody::TYPE_BROWN_DWARF)) {
		// stars should emit light and terrain should be visible from distance
		ambient.r = ambient.g = ambient.b = 51;
		ambient.a = 255;
		emission.r = StarSystem::starRealColors[m_sbody->m_type][0];
		emission.g = StarSystem::starRealColors[m_sbody->m_type][1];
		emission.b = StarSystem::starRealColors[m_sbody->m_type][2];
		emission.a = 255;
	}
...
void GeoSphere::SetUpMaterials()
{
	...
	if ((m_sbody->m_type == SystemBody::TYPE_BROWN_DWARF) ||
		(m_sbody->m_type == SystemBody::TYPE_STAR_M)) {
