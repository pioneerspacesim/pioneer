...
static int l_starsystem_get_nearby_systems(lua_State *l)
{
	...
	const Sector* here_sec = Sector::cache.GetCached(here_x, here_y, here_z);
	...
	for (int x = here_x-diff_sec; x <= here_x+diff_sec; x++) {
		for (int y = here_y-diff_sec; y <= here_y+diff_sec; y++) {
			for (int z = here_z-diff_sec; z <= here_z+diff_sec; z++) {
				const Sector* sec = Sector::cache.GetCached(x, y, z);
...
static int l_starsystem_distance_to(lua_State *l)
{
	...
	const Sector* sec1 = Sector::cache.GetCached(loc1->sectorX, loc1->sectorY, loc1->sectorZ);
	const Sector* sec2 = Sector::cache.GetCached(loc2->sectorX, loc2->sectorY, loc2->sectorZ);
