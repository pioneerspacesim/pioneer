...
namespace Graphics {
	...
	public:
		...
		// The specified offsets in the buffer bindings are elements, not bytes.
		virtual void Draw(Span<const BufferBinding<VertexBuffer>> vtxBuffers, BufferBinding<IndexBuffer> idx, Material *m, uint32_t numElements, uint32_t numInstances = 1) = 0;
		...
		// Allocate and return a frame-temporary vertex buffer binding for the given vertex data.
		// Intended to be used for per-frame instance data and VertexArray buffers.
		// The vertex data is not guaranteed to be synced to the GPU until FlushCommandBuffers() has been called.
		// The BufferUsage parameter is implied to be BUFFER_USAGE_DYNAMIC.
		virtual BufferBinding<VertexBuffer> CreateTempVertexBuffer(uint32_t numElements, uint32_t stride) = 0;
