...
// CloudSphereProgram -------------------------------------------
CloudSphereProgram::CloudSphereProgram(const std::string &filename, const std::string &defines)
{
	m_name = filename;
	m_defines = defines;
...
void CloudSphereProgram::InitUniforms()
{
...
Program *CloudSphereMaterial::CreateProgram(const MaterialDescriptor &desc)
{
	...
	if(desc.dirLights>0) {
		...
		ss << stringf("#define INV_NUM_LIGHTS 1.0\n");
	}
	if (desc.textures > 0)
		ss << "#define TEXTURE0\n";
...
void CloudSphereMaterial::SetGSUniforms()
{
	OGL::Material::Apply();

	CloudSphereProgram *p = static_cast<CloudSphereProgram*>(m_program);
