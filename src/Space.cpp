...
void Space::AddSystemBodyToIndex(SystemBody *sbody)
{
	...
	for (Uint32 i = 0; i < sbody->m_children.size(); i++)
		AddSystemBodyToIndex(sbody->m_children[i]);
}
...
vector3d Space::GetHyperspaceExitPoint(const SystemPath &source, const SystemPath &dest) const
{
	...
	if (dest.IsBodyPath()) {
		...
		while (primary && primary->GetSystemBody()->GetSuperType() != SystemBody::SUPERTYPE_STAR) {
			SystemBody* parent = primary->GetSystemBody()->m_parent;
			primary = parent ? FindBodyForPath(&parent->m_path) : 0;
		}
	}
	if (!primary) {
		// find the first non-gravpoint. should be the primary star
		for (BodyIterator i = BodiesBegin(); i != BodiesEnd(); ++i)
			if ((*i)->GetSystemBody()->m_type != SystemBody::TYPE_GRAVPOINT) {
				primary = *i;
				break;
			}
	}
...
static void RelocateStarportIfUnderwaterOrBuried(SystemBody *sbody, Frame *frame, Planet *planet, vector3d &pos, matrix3x3d &rot)
{
	...
	bool manualRelocationIsEasy = !(planet->GetSystemBody()->m_type == SystemBody::TYPE_PLANET_ASTEROID || terrainHeightVariation > heightVariationCheckThreshold);
	...
	const bool isRelocatableIfBuried = !(sbody->m_isCustomBody && manualRelocationIsEasy);

	bool isInitiallyUnderwater = false;
	bool initialVariationTooHigh = false;

	Random r(sbody->m_seed);
	...
	if (sbody->m_isCustomBody) {
		SystemPath &p = sbody->m_path;
		if (initialVariationTooHigh) {
			if (isRelocatableIfBuried) {
				Output("Warning: Lua custom Systems definition: Surface starport has been automatically relocated. This is in order to place it on flatter ground to reduce the chance of landing pads being buried. This is not an error as such and you may attempt to move the starport to another location by changing latitude and longitude fields.\n      Surface starport name: %s, Body name: %s, In sector: x = %i, y = %i, z = %i.\n",
					sbody->m_name.c_str(), sbody->m_parent->m_name.c_str(), p.sectorX, p.sectorY, p.sectorZ);
			} else {
...
static Frame *MakeFrameFor(double at_time, SystemBody *sbody, Body *b, Frame *f)
{
	...
	if (sbody->m_type == SystemBody::TYPE_GRAVPOINT) {
		Frame *orbFrame = new Frame(f, sbody->m_name.c_str());
	...
	if ((supertype == SystemBody::SUPERTYPE_GAS_GIANT) ||
		...
		Frame *orbFrame = new Frame(f, sbody->m_name.c_str(), Frame::FLAG_HAS_ROT);
		orbFrame->SetBodies(sbody, b);
		...
		assert(sbody->m_rotationPeriod != 0);
		Frame *rotFrame = new Frame(orbFrame, sbody->m_name.c_str(), Frame::FLAG_ROTATING);
		...
		matrix3x3d rotMatrix = matrix3x3d::RotateX(sbody->m_axialTilt.ToDouble());
		double angSpeed = 2.0*M_PI/sbody->GetRotationPeriod();
		rotFrame->SetAngSpeed(angSpeed);

		if (sbody->m_rotationalPhaseAtStart != fixed(0))
			rotMatrix = rotMatrix * matrix3x3d::RotateY(sbody->m_rotationalPhaseAtStart.ToDouble());
		...
		if (sbody->m_parent && frameRadius > AU * 0.11 * sbody->m_orbMin.ToDouble())
			frameRadius = std::max(1.1*sbody->GetRadius(), AU * 0.11 * sbody->m_orbMin.ToDouble());
		orbFrame->SetRadius(frameRadius);
		b->SetFrame(orbFrame);
		return orbFrame;
	}
	else if (sbody->m_type == SystemBody::TYPE_STARPORT_ORBITAL) {
...
//		b->SetFrame(rotFrame);
	...
	} else if (sbody->m_type == SystemBody::TYPE_STARPORT_SURFACE) {
	...
	if (sbody->m_type != SystemBody::TYPE_GRAVPOINT) {
		if (sbody->GetSuperType() == SystemBody::SUPERTYPE_STAR) {
			...
			b = star;
		           ...
		           (sbody->m_type == SystemBody::TYPE_STARPORT_SURFACE)) {
		...
		b->SetLabel(sbody->m_name.c_str());
		b->SetPosition(vector3d(0,0,0));
		AddBody(b);
	}
	f = MakeFrameFor(at_time, sbody, b, f);

	for (std::vector<SystemBody*>::iterator i = sbody->m_children.begin(); i != sbody->m_children.end(); ++i) {
		GenBody(at_time, *i, f);
	}
}
