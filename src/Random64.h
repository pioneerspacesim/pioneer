...
public:
	...
	// interval [min, max]
	inline Uint64 Int64(const long long min, const long long max)
	{
		return (Int64() % (1 + max - min)) + min;
	}

	// Pick a number in the half open interval [0, 1)
	inline double Double()
	{
		return double(Int64()) * (1. / 4294967296.); // divided by 2^32
	}

	// Pick a number in the closed interval [0, 1]
	inline double Double_closed()
	{
		return double(Int64()) * (1. / 4294967295.); // divided by 2^32 - 1
	}

	// Pick a number in the open interval (0, 1)
	inline double Double_open()
	{
		return (double(Int64()) + .5) * (1. / 4294967296.); // divided by 2^32
	}
	...
	{
		return (double(Int64() >> 5) * 67108864. + double(Int64() >> 6)) * (1. / 9007199254740992.);
	}
...
private:
	...
	void operator=(const Random64&); // assignment operator not defined
};
