...
void SystemView::OnClickObject(const SystemBody *b)
{
	...
	desc += ":\n";
	data += stringf(Lang::N_DAYS, formatarg("days", b->m_rotationPeriod.ToFloat())) + "\n";
	...
	desc += ":\n";
		data += format_distance(b->m_orbit.GetSemiMajorAxis())+"\n";

		desc += std::string(Lang::ORBITAL_PERIOD);
	desc += ":\n";
		data += stringf(Lang::N_DAYS, formatarg("days", b->m_orbit.Period() / (24*60*60))) + "\n";
	}
...
void SystemView::PutLabel(const SystemBody *b, const vector3d &offset)
{
	...
	if (Gui::Screen::Project(offset, pos)) {
		...
		m_objectLabels->Add(b->m_name, sigc::bind(sigc::mem_fun(this, &SystemView::OnClickObject), b), pos.x, pos.y);
	}
...
{
	if (b->m_type == SystemBody::TYPE_STARPORT_SURFACE) return;
	if (b->m_type != SystemBody::TYPE_GRAVPOINT) {
	...
	if (b->m_children.size()) {
		for(std::vector<SystemBody*>::const_iterator kid = b->m_children.begin(); kid != b->m_children.end(); ++kid) {
			if (is_zero_general((*kid)->m_orbit.GetSemiMajorAxis())) continue;
			if ((*kid)->m_orbit.GetSemiMajorAxis() * m_zoom < ROUGH_SIZE_OF_TURD) {
				PutOrbit(&((*kid)->m_orbit), offset, Color(0, 255, 0, 255));
			}

			// not using current time yet
			vector3d pos = (*kid)->m_orbit.OrbitalPosAtTime(m_time);
			pos *= double(m_zoom);
...
void SystemView::PutSelectionBox(const SystemBody *b, const vector3d &rootPos, const Color &col)
{
	...
	if (b->m_type == SystemBody::TYPE_STARPORT_SURFACE)
		b = b->m_parent;
	assert(b);
	...
	while (b->m_parent) {
		pos += b->m_orbit.OrbitalPosAtTime(m_time) * double(m_zoom);
		b = b->m_parent;
	}
...
{
	if (b->m_parent) {
		GetTransformTo(b->m_parent, pos);
		pos -= double(m_zoom) * b->m_orbit.OrbitalPosAtTime(m_time);
	}
}
