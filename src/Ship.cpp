...
double Ship::GetAccelMin() const
{
	...
	val = Min(val, m_type->linThrust[ShipType::THRUSTER_RIGHT]);
	val = Min(val, -m_type->linThrust[ShipType::THRUSTER_LEFT]);
	return val / GetMass();
}
...
void Ship::StaticUpdate(const float timeStep)
{
	...
	if (m_ecmRecharge > 0.0f) {
		m_ecmRecharge = Max(0.0f, m_ecmRecharge - timeStep);
	}

	if (m_shieldCooldown > 0.0f) {
		m_shieldCooldown = Max(0.0f, m_shieldCooldown - timeStep);
	}
	...
	if (m_equipment.Get(Equip::SLOT_HULLAUTOREPAIR) == Equip::HULL_AUTOREPAIR) {
		m_stats.hull_mass_left = Min(m_stats.hull_mass_left + 0.1f*timeStep, float(m_type->hullMass));
		Properties().Set("hullMassLeft", m_stats.hull_mass_left);
		Properties().Set("hullPercent", 100.0f * (m_stats.hull_mass_left / float(m_type->hullMass)));
	}
	...
	if (m_type->tag == ShipType::TAG_MISSILE && m_thrusters.z < 0.0 && 0.1*Pi::rng.Double() < timeStep) {
		...
		Sfx::AddThrustSmoke(this, Sfx::TYPE_SMOKE, Min(10.0*GetVelocity().Length()*abs(m_thrusters.z),100.0),pos);
	}
}
