...
public:
	...
	static AICommand *LoadFromJson(const Json::Value &jsonObj); // npw - new code (under construction)
	AICommand(Serializer::Reader &rd, CmdName name);
	AICommand(const Json::Value &jsonObj, CmdName name); // npw - new code (under construction)
...
public:
	...
	AICmdDock(const Json::Value &jsonObj) : AICommand(jsonObj, CMD_DOCK) { // npw - new code (under construction)
...
public:
	...
	AICmdFlyTo(const Json::Value &jsonObj) : AICommand(jsonObj, CMD_FLYTO) { // npw - new code (under construction)
		if (!jsonObj.isMember("index_for_target")) throw SavedGameCorruptException();
...
public:
	...
	AICmdFlyAround(const Json::Value &jsonObj) : AICommand(jsonObj, CMD_FLYAROUND) { // npw - new code (under construction)
...
public:
	...
	AICmdKill(const Json::Value &jsonObj) : AICommand(jsonObj, CMD_KILL) { // npw - new code (under construction)
		if (!jsonObj.isMember("index_for_target")) throw SavedGameCorruptException();
		m_targetIndex = jsonObj["index_for_target"].asInt();
	}
	virtual void PostLoadFixup(Space *space) {
...
public:
	...
	AICmdKamikaze(const Json::Value &jsonObj) : AICommand(jsonObj, CMD_KAMIKAZE) { // npw - new code (under construction)
		if (!jsonObj.isMember("index_for_target")) throw SavedGameCorruptException();
		m_targetIndex = jsonObj["index_for_target"].asInt();
	}
	virtual void PostLoadFixup(Space *space) {
...
public:
	...
	AICmdHoldPosition(const Json::Value &jsonObj) : AICommand(jsonObj, CMD_HOLDPOSITION) { } // npw - new code (under construction)
};
...
public:
	...
	AICmdFormation(const Json::Value &jsonObj) : AICommand(jsonObj, CMD_FORMATION) { // npw - new code (under construction)
		if (!jsonObj.isMember("index_for_target")) throw SavedGameCorruptException();
		if (!jsonObj.isMember("pos_off")) throw SavedGameCorruptException();
		m_targetIndex = jsonObj["index_for_target"].asInt();
		JsonToVector(&m_posoff, jsonObj, "pos_off");
